#!/usr/bin/env python3
"""
ACE + GitHub Copilot: Simple Learning Loop

GitHub Copilot works autonomously until done, using .agent/ as scratchpad.
...
Usage:
    python copilot_loop.py                    # Interactive mode
    AUTO_MODE=true python copilot_loop.py     # Fully automatic
"""
...
# Load ACE loop config from .env.copilot (not workspace .env)
load_dotenv(Path(__file__).parent / ".env.copilot")

# Configuration
AUTO_MODE = os.getenv("AUTO_MODE", "true").lower() == "true"
ACE_MODEL = os.getenv("ACE_MODEL", "gpt-4o-mini")
...
def main():
    ...
    while True:
        ...
        # Run Copilot - let it work until done
        prompt = load_prompt()
